...
const AlertForm = () => {
  ...
  // NEW: capture lat/lon AND fill Location with a human-readable address (fallback to "lat, lon")
const getCurrentLocation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('/api/alerts', submitData, {
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.error || 'Failed to submit alert. Please try again.'
      });
    } finally {
      ...
      <div className="form-container">
        <div className="text-center mb-5">
          ...
          <p className="lead text-muted">
            Help us spread the word about your missing loved one
          </p>
        </div>
